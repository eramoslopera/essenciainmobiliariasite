...
const ContactForm: React.FC<ContactFormProps> = ({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Persist to Supabase (non-blocking — silently degrades if not configured)
    ...
    window.open(
      `https://wa.me/34647803355?text=${encodeURIComponent(waMessage)}`,
      '_blank',
      'noopener,noreferrer'
    );
      ...
      {sent ? (
        <div className="flex flex-col items-center justify-center h-full min-h-[320px] gap-4 text-center">
          ...
          <p
            className={`font-bold text-lg ${
              isDark ? 'text-white' : 'text-editorial-black'
            }`}
          >
            ¡WhatsApp abierto!
          </p>
          <p className={`text-sm ${isDark ? 'text-gray-400' : 'text-gray-500'}`}>
            Envía el mensaje para que os podamos contactar.
          </p>
        </div>
      ) : (
        <form onSubmit={handleSubmit} className="space-y-5" noValidate>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
            <div>
              <label htmlFor={`form-name-${source}`} className={labelClasses}>
            ...
            <div>
              <label htmlFor={`form-phone-${source}`} className={labelClasses}>
                {t('common.phone') || 'Teléfono'}
              </label>
              <input
                id={`form-phone-${source}`}
                type="tel"
                value={phone}
          ...
          <div>
            <label htmlFor={`form-email-${source}`} className={labelClasses}>
          ...
          <div>
            <label htmlFor={`form-reason-${source}`} className={labelClasses}>
              {t('home.contact.reason.label')}
            </label>
            <select
              id={`form-reason-${source}`}
              value={reason}
              onChange={(e) => setReason(e.target.value)}
              className={`${inputClasses} appearance-none`}
              aria-label={t('home.contact.reason.label')}
            >
              <option value="buy" className={isDark ? 'bg-editorial-black' : ''}>
                {t('home.contact.reason.buy')}
              </option>
              <option value="sell" className={isDark ? 'bg-editorial-black' : ''}>
                {t('home.contact.reason.sell')}
              </option>
              <option value="valuation" className={isDark ? 'bg-editorial-black' : ''}>
                {t('home.contact.reason.valuation')}
              </option>
              <option value="invest" className={isDark ? 'bg-editorial-black' : ''}>
                {t('home.contact.reason.invest')}
              </option>
              <option value="other" className={isDark ? 'bg-editorial-black' : ''}>
                {t('home.contact.reason.other')}
              </option>
            </select>
          </div>

          <div>
            <label htmlFor={`form-message-${source}`} className={labelClasses}>
              {t('common.message')}
            </label>
            <textarea
              id={`form-message-${source}`}
              rows={4}
              value={message}
